...
def encode_user_item(df, user_col, item_col, rating_col, time_col):
    ...
    encoded_df.rename(
        {rating_col: "RATING", time_col: "TIMESTAMP"}, axis=1, inplace=True
    )
...
def user_split(df, col_time, col_user, ratios, chrono=False):
    ...
    splits_list = [
        splits_all[splits_all["split_index"] == x] for x in range(len(ratios))
    ]

    return splits_list


def neg_feedback_samples(df, rating_threshold, ratio_neg_per_user=1):
    ...
    df_pos = df.copy()
    df_pos["RATING"] = df_pos["RATING"].apply(
                    ...
                    (
                        x[x["RATING"] == 0].sample(
                            min(
                                max(
                                    round(
                                        len(x[x["RATING"] == 1]) * ratio_neg_per_user
                                    ),
                                    1,
                                ),
                                len(x[x["RATING"] == 0]),
