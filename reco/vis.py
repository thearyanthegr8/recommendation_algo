...
def metrics(history):
    ...
    base = (
        alt.Chart()
        .encode(x="batch:Q", y="value:Q", color="type")
        .properties(width=300, height=300)
    )

    layers = (
        base.mark_circle(size=50).encode(tooltip=["batch", "value"]) + base.mark_line()
    )
...
def predict(proba, actual, labels):
    ...
    chart = (
        ...
        .properties(
            width=350, height=350, title="Prediction: " + labels[predicted_class]
        )
    )
    return chart
